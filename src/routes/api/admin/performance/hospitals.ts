...
import { formatRate, rankHospitals, MIN_LEADS_FOR_RANKING } from "@/lib/performance-logic";

// GET|POST /api/admin/performance/hospitals?from=&to=
...
async function handle(request: Request) {
  ...
  try {
    ...
    const rows = rankHospitals(ds).map((r) => ({
      ...r,
    ...
    console.error("performance/hospitals error:", err);
    return json({ error: err instanceof Error ? err.message : "Query failed" }, 500);
  }
}

export const Route = createFileRoute("/api/admin/performance/hospitals")({
  server: {
    handlers: {
